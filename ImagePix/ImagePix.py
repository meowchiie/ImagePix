...
def apply_process():
    ...
    elif choice == "Operasi OR":
        dummy = np.ones_like(img) * 127
        processed_img = cv2.bitwise_or(img, dummy.astype(np.uint8))
    elif choice == "Sharpen":
...
# UI layout
judul_text = Label(root, text="ImagePix ✨", font=("Segoe UI", 24, "bold"), fg="#833FC2", bg="white")
judul_text.pack(pady=(20, 10))
...
options = [
    "Grayscale", "Binary", "+ Brightness", "- Brightness",
    "Operasi AND", "Operasi OR", "Sharpen", "Dilasi"
]
dropdown = OptionMenu(root, selected_option, *options)
dropdown.config(width=20, font=("Segoe UI", 10))
dropdown.pack(pady=5)

Button(root, text="Terapkan Proses", command=apply_process, bg="#F4B3DB", font=("Segoe UI", 10, "bold")).pack(pady=5)

# Tombol lainnya
action_frame = Frame(root, bg="white")
action_frame.pack(pady=10)

Button(action_frame, text="Upload Gambar", command=upload_gambar, width=18, bg="#F4B3DB").grid(row=0, column=0, padx=5, pady=5)
Button(action_frame, text="Histogram RGB", command=tampilkan_histogram, width=18, bg="#F4B3DB").grid(row=0, column=3, padx=5, pady=5)
